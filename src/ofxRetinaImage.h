...
public:
	...
	void draw(float x, float y){
        if (isRetina) {
            drawSubsection(x, y, 0, getWidth() * scale, getHeight() * scale, 0,0, getWidth() / scale, getHeight() / scale);
        }
        else {
            drawSubsection(x, y, 0, getWidth() * .5, getHeight() * .5, 0,0, getWidth() / scale, getHeight() / scale);
        }
    }
